...
import com.tricolorflower.heartbeat.voiceroom.presenter.EmojiPagerPresenter;
 ...
 * @Description
 */
public class EmojiPagerFragment extends QsViewPagerFragment<EmojiPagerPresenter> {
    ...
    public void updateViewPager(List<EmojiPageList.EmojiPage> emojiPages) {
        if (emojiPages == null)
            return;

        List<QsModelPager> modelPagers = new ArrayList<>();
        for (int i = 0; i < emojiPages.size(); i++) {
            QsModelPager model = createModelPager(i, emojiPages.get(i).pageId + "");
            Bundle bundle = new Bundle();
            bundle.putInt(VoiceRoomConstants.BUNDLE_KEY_PRODUCTLIST_REQUEST_PAGE_NO, i + 1);
            model.fragment = EmojiGridFragment.getInstance(bundle);
    ...
    public int getTabItemLayout() {
        return R.layout.item_emoji_pager_tab;
    }
