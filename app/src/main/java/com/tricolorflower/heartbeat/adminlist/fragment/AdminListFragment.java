...
public class AdminListFragment extends QsPullRecyclerFragment<AdminListPresenter, VoiceRole> implements ItemListener<VoiceRole> {
    ...
    public static AdminListFragment getInstance(Bundle extras) {
    ...
    public QsRecycleAdapterItem getRecycleAdapterItem(LayoutInflater mInflater, ViewGroup parent, int type) {
        return new AdminRecyclerAdapterItem(mInflater, parent, this);
    }
    ...
    public void onItemClick(VoiceRole data, int position, int totalCount) {
        ...
        getPresenter().deleteAdmin(token, data.id);
    }


    @Subscribe
    public void onEvent(VoiceRoleOperationEvent.OnAddAdminEvent event) {
    ...
    @Subscribe
    public void onEvent(VoiceRoleOperationEvent.OnRemoveAdminEvent event) {
        if (event == null || event.data == null)
            return;
