...
public class LocalImageLoader {
     ...
     * 图片请求列表，用于调度
     */
    private final ArrayList<ImageRequest> mImagesList = new ArrayList<>();

    /**
     * 处于正在请求状态的请求列表
     */
    private final ArrayList<ImageRequest> mOnLoadingList = new ArrayList<>();
        ...
        mThreadPool.execute(new Runnable() {
            ...
            public void run() {
                ...
                if (size == null || size.x == 0 || size.y == 0) {
                    size = new Point(CommonUtils.getScreenWidth(), CommonUtils.getScreenHeight());
                }
                // 先获取图片的缩略图
     ...
     * and charHeight equal to or larger than the requested width and charHeight. This
     ...
     * @param reqHeight The requested charHeight of the resulting bitmap
     * @return The value to be used for inSampleSize
     */
    private int computeScale(BitmapFactory.Options options, int reqWidth, int reqHeight) {
        // Raw charHeight and width of image
        final int height = options.outHeight;
        final int width = options.outWidth;
        int inSampleSize = 1;

        if (height > reqHeight || width > reqWidth) {
            // Calculate ratios of charHeight and width to requested charHeight and
            // width
            ...
            // charHeight and width.
            inSampleSize = heightRatio < widthRatio ? heightRatio : widthRatio;

            // This offers some additional logic in case the image has a strange
            // aspect ratio. For example, a panorama may have a much larger
            // width than charHeight. In these cases the total pixels might still
    ...
    private static class ImageHandler extends Handler {
        ...
        private ImageHandler(LocalImageLoader activity, ImageRequest request) {
     ...
     */
    public static ImageCallBack getImageListener(final ImageView view, final Object tag, final int defaultImageResId, final int errorImageResId) {
        if (view != null) {
